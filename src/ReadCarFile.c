...
/* ----------------------------------------------------------------------
   ...
   assumes box's lo/hi and triclinic tilts are already set
------------------------------------------------------------------------- */
...
void ReadCarFile(void)
{
  ...
  if (periodic == 0) {
    ...
    if (TriclinicFlag == 0) {
      ...
      sin_gamma = sin(pbc[5]*PI_180); /* adjusted by HZ */
      cos_beta =  cos(pbc[4]*PI_180); /* adjusted by HZ */
      if (pflag > 2) {
      ...
      center[0] += (double) -0.5*ly*cos_gamma/sin_gamma; /* added by HZ */
      box[0][0] = -0.5*A + center[0] + shift[0]; /* added by HZ */
      box[1][0] =  0.5*A + center[0] + shift[0]; /* added by HZ */
